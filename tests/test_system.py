...
class DetectionTests(unittest.TestCase):
    def detect(self, release, commands):
        with tempfile.TemporaryDirectory() as temp:
            ...
            binaries = pathlib.Path(temp) / 'bin'
            binaries.mkdir()
            for command in commands:
                executable = binaries / command
                executable.write_text('#!/bin/bash\nexit 0\n')
                executable.chmod(0o755)
            result = subprocess.run(['/bin/bash', '-c', 'source "$1"; update_backend "$2"',
                                     'bash', str(ROOT / 'install.sh'), str(path)],
                                    env=dict(os.environ, PATH=str(binaries)), capture_output=True, text=True)
    ...
    def test_single_quoted_id_without_final_newline(self):
        self.assertEqual(self.detect("ID='arch'", {'pacman'}), 'pacman')

    def test_missing_pacman(self):
        with self.assertRaisesRegex(RuntimeError, 'pacman command is missing'):
            self.detect('ID=arch\n', set())
...
class InstallerTests(unittest.TestCase):
    def run_installer(self, backend, update_status=0, args=('orca',), input_text=''):
        with tempfile.TemporaryDirectory() as temp:
            ...
            release.write_text('ID=omarchy\n' if backend == 'omarchy' else 'ID=arch\n')
            (root / 'install.sh').write_text((ROOT / 'install.sh').read_text().replace('/etc/os-release', str(release)))
            binaries = root / 'bin'
            binaries.mkdir()
            ...
            if backend == 'omarchy':
                scripts['omarchy'] = 'printf "omarchy %s\\n" "$*" >> "$TEST_LOG"\nexit "$TEST_UPDATE_STATUS"\n'
